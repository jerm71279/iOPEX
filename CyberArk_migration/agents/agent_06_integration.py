...
replacement code for Privilege Cloud, and optionally creates dual-backend
...
# Privilege Cloud replacement templates
REPLACEMENT_TEMPLATES = {
    "python": '''import requests

def get_secret(base_url, token, safe, account_name):
    """Retrieve secret from CyberArk Privilege Cloud."""
    url = f"{{base_url}}/PasswordVault/api/Accounts?search={{account_name}}&filter=safeName eq {{safe}}"
    headers = {{"Authorization": token, "Content-Type": "application/json"}}
    resp = requests.get(url, headers=headers)
    resp.raise_for_status()
    accounts = resp.json().get("value", [])
    if not accounts:
        raise ValueError(f"Account {{account_name}} not found in {{safe}}")
    account_id = accounts[0]["id"]
    pwd_url = f"{{base_url}}/PasswordVault/api/Accounts/{{account_id}}/Password/Retrieve"
    pwd_resp = requests.post(pwd_url, headers=headers, json={{"reason": "Application access"}})
    pwd_resp.raise_for_status()
    return pwd_resp.text.strip('"')
''',
    "powershell": '''function Get-PrivilegeCloudSecret {
    param(
        [string]$BaseUrl,
        [string]$Token,
        [string]$Safe,
    ...
    $headers = @{ Authorization = $Token; "Content-Type" = "application/json" }
    $searchUrl = "$BaseUrl/PasswordVault/api/Accounts?search=$AccountName&filter=safeName eq $Safe"
    $result = Invoke-RestMethod -Uri $searchUrl -Headers $headers -Method Get
    $accountId = $result.value[0].id
    $pwdUrl = "$BaseUrl/PasswordVault/api/Accounts/$accountId/Password/Retrieve"
...
public class PrivilegeCloudClient
{
    private readonly HttpClient _client;
    private readonly string _baseUrl;

    public PrivilegeCloudClient(string baseUrl, string token)
    {
        _baseUrl = baseUrl;
    ...
    public async Task<string> GetSecretAsync(string safe, string accountName)
    {
        var searchUrl = $"{_baseUrl}/PasswordVault/api/Accounts?search={accountName}&filter=safeName eq {safe}";
        var searchResp = await _client.GetStringAsync(searchUrl);
        var accounts = JsonSerializer.Deserialize<AccountsResponse>(searchResp);
        var accountId = accounts.Value[0].Id;
        var pwdUrl = $"{_baseUrl}/PasswordVault/api/Accounts/{accountId}/Password/Retrieve";
        ...
        return await pwdResp.Content.ReadAsStringAsync();
    }
}
''',
...
public class PrivilegeCloudClient {
    ...
    public String getSecret(String safe, String accountName) throws Exception {
        String searchUrl = baseUrl + "/PasswordVault/api/Accounts?search=" + accountName + "&filter=safeName eq " + safe;
        HttpRequest searchReq = HttpRequest.newBuilder()
            .uri(URI.create(searchUrl))
            .header("Authorization", token)
            .GET().build();
        HttpResponse<String> searchResp = client.send(searchReq, HttpResponse.BodyHandlers.ofString());
        // Parse JSON to get account ID, then retrieve password
        String accountId = parseAccountId(searchResp.body());
        String pwdUrl = baseUrl + "/PasswordVault/api/Accounts/" + accountId + "/Password/Retrieve";
        ...
        return pwdResp.body().replace("\\"", "");
    }

    private String parseAccountId(String json) {
        // Simplified - use a proper JSON parser in production
...
class IntegrationRepointingAgent(AgentBase):
    """Scans for CyberArk integrations and generates Privilege Cloud replacements."""
