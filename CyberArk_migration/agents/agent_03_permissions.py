...
Maps CyberArk safe member permissions directly to Privilege Cloud using
individual permissions (NOT a simplified role model). Privilege Cloud
uses the same granular permission model as on-prem via the Safe Members API.

Detects missing permissions, flags security-sensitive permission holders,
and in Phase P3 actually applies permissions to target safes.

Phases:
    P1: Analyze permissions from discovery data, produce translation report
    P3: Apply permission mappings to migrated safes in Privilege Cloud
"""
...
class PermissionMappingAgent(AgentBase):
    """Maps individual CyberArk permissions directly to Privilege Cloud."""
    ...
    def run(self, phase: str, input_data: dict) -> AgentResult:
        ...
        if phase == "P3":
            # Actually apply permissions to Privilege Cloud
    ...
    def _apply_permissions(self, analysis: dict, cfg: dict) -> dict:
        """Apply permission mappings to Privilege Cloud safes (Phase P3)."""
        cloud_cfg = self.config.get("privilege_cloud", {})
        if not cloud_cfg.get("base_url"):
            return {"status": "skipped", "reason": "privilege_cloud not configured"}
            ...
            with CloudClient(cloud_cfg) as client:
                for safe_name, members in analysis["translations"].items():
                    for member_info in members:
                        ...
                        # Build Privilege Cloud safe member payload
                        payload = {
