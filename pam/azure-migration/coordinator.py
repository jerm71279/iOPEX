...
class Coordinator:
    ...
    def run_phase(self, phase: str):
        ...
        for agent_key in agents_to_run:
            ...
            # Push live status to stakeholder dashboard after each agent
    ...
    def _export_dashboard(self):
        """Non-blocking dashboard status export — failure never stops migration."""
        try:
            from core.dashboard_export import DashboardExporter
            output_dir = self.config.get("output_dir", "./output")
            exporter = DashboardExporter(self.config, self.state, f"{output_dir}/state")
            exporter.export()
        except Exception as e:
            logger.debug("Dashboard export skipped: %s", e)
