"""Synthetic data generators for ML module testing and cold-start validation.

Produces realistic ETL step-timing records and labeled NHI classification
examples.  Used by unit tests and by WaveLearningCoordinator for cold-start
seeding when no historical data exists.
"""
...
# Baseline durations (seconds) and standard deviations per ETL step
...
def generate_etl_timing_data(n: int = 200, seed: int = 42) -> List[Dict]:
    ...
    Returns a list of dicts, each representing one step execution with:
        step_name, duration, account_count, nhi_ratio,
        platform_complexity, batch_position, wave, is_anomaly (ground truth).
    """
    ...
    for i in range(n):
        ...
        # Wave 4/5 (NHI-heavy) runs ~40 % slower
...
_NHI_PLATFORMS = [
    ...
    "AzureServicePrincipal", "OracleDB", "MSSql",
]
_HUMAN_PLATFORMS = [
    "WinServerLocal", "WinDomain", "UnixSSH", "MySQL",
]

_NHI_NAME_PREFIXES = ["svc-", "svc_", "app-", "app_", "bot-", "rpa-", "api-"]
_HUMAN_NAME_PREFIXES = ["john.", "jane.", "admin.", "user.", "mgr.", "dev."]

_NHI_SAFE_PATTERNS = ["appcred", "automation", "cicd", "pipeline", "svc"]
_HUMAN_SAFE_PATTERNS = ["it-admins", "finance", "helpdesk", "security"]


def generate_nhi_labeled_data(n: int = 150, seed: int = 42) -> List[Dict]:
    """Generate *n* labeled NHI classification examples.

    Distribution: ~40 % NHI, ~60 % human.
    Includes ~15 % borderline cases and ~5 % conflicting-signal cases.

    Returns list of dicts with:
        platform_signal, name_signal, container_signal,
        dependency_signal, audit_signal, account_name_length,
        safe_depth, has_linked_accounts, is_nhi (ground truth).
    """
    rng = random.Random(seed)
    records: List[Dict] = []
    ...
    for i in range(n):
        ...
        # Decide if this is a borderline or conflicting case
        ...
        if is_nhi:
            ...
            safe_pattern = rng.choice(_NHI_SAFE_PATTERNS)
        else:
            ...
            safe_pattern = rng.choice(_HUMAN_SAFE_PATTERNS)

        if is_borderline:
            # Push signals toward the 0.4-0.6 range
            platform_signal = rng.uniform(0.35, 0.65)
            name_signal = rng.uniform(0.35, 0.65)

        if is_conflicting:
            # High platform signal but low name signal (or vice versa)
            if is_nhi:
