...
Runs a full mini-ETL cycle against a STAGING KeeperPAM tenant to validate
...
class StagingValidationAgent(AgentBase):
    """Staging validation harness for CyberArk-to-KeeperPAM migration.

    Runs a stratified sample through the full ETL pipeline against a staging
    KeeperPAM tenant, then executes 10 assertions to validate correctness.
    ...
    def preflight(self) -> AgentResult:
        ...
        if not staging_url:
            errors.append(
                ...
                "A dedicated staging KeeperPAM tenant is required."
            )

        # 2. Staging != production (HARD BLOCK — prevents accidental production writes)
        prod_cfg = self.config.get("keeperpam", {})
        ...
        if staging_url and prod_url:
            ...
            if staging_normalized == prod_normalized:
                errors.append(
                    "HARD BLOCK: staging.base_url matches keeperpam.base_url. "
    ...
    def _run_mini_etl(
        ...
        try:
            ...
            # 3. Transform to KeeperPAM format
    ...
    def _transform_accounts(self, accounts: List[dict]) -> List[dict]:
        """Transform CyberArk account format to KeeperPAM format.
