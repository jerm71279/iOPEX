...
15-agent AI orchestrator executing CyberArk PAS on-premises → KeeperPAM migration. The coordinator sequences agents through 8 phases (P0–P7), gating each phase on prior agent output. State is persisted atomically with SHA-256 audit chain for crash recovery and compliance. Source: CyberArk PVWA REST API. Target: KeeperPAM REST API + OAuth2.
...
core/
  ...
  keeper_client.py          KeeperPAM REST client (target) — OAuth2 client_credentials
...
agents/
  ...
  agent_06_integration.py   Rewirer   — CCP/AAM code scan + repoint to KeeperPAM
...
PHASE P3 — POLICY MIGRATION
  → Agent 03 (Mapper) re-run → safe creation in KeeperPAM
...
| KeeperPAM API | HTTP/JSON | Outbound (target) | KeeperPAM REST API; OAuth2 client_credentials |
...
- **Credentials**: `CYBERARK_USERNAME`, `CYBERARK_PASSWORD`, `KEEPERPAM_CLIENT_ID`, `KEEPERPAM_CLIENT_SECRET` via env vars only. Passwords zeroed from memory after auth.
- **config.json**: gitignored — NEVER commit. Contains live connection URLs.
- **HTTPS enforced**: KeeperPAM requires TLS. PVWA enforced in production.
...
|----------|--------|-----------|
| Permission model | 22→22 (1:1, no loss) | KeeperPAM uses identical 22-permission model — no translation layer needed |
| Auth: KeeperPAM | OAuth2 client_credentials | Preferred for programmatic access; supports token refresh without user interaction |
