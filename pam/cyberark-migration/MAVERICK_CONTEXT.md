...
| Field | Value |
|-------|-------|
| Project | CyberArk Migration — Option A (Privilege Cloud) |
...
- PVWA REST client (source) + Privilege Cloud REST client (target) with OAuth2
...
- **No test sandbox**: No dedicated CyberArk PVWA dev/lab or Privilege Cloud test tenant. All testing requires production-like environment. See README.md for options.
...
- **Option A (Privilege Cloud) vs Option B (Secret Server)**: This project is Option A. Privilege Cloud preserves the 22-permission model 1:1 — no permission loss. Secret Server (Option B) collapses to 4 roles with lossy mapping.
