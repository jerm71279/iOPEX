...
| `privilege_cloud.auth_method` | `"oauth2"` (modern, recommended) or `"legacy"` (PVWA logon) |
| `privilege_cloud.identity_url` | CyberArk Identity URL for OAuth2 (`https://<tenant>.id.cyberark.cloud`) |
| `privilege_cloud.batch_size` | Import pagination size (default: `500`, max: `1000`) |
...
**OAuth2 authentication** (modern Privilege Cloud tenants):
```
identity_url: https://<tenant>.id.cyberark.cloud
Token endpoint: {identity_url}/oauth2/platformtoken
...
- HTTPS enforced for Privilege Cloud connections
...
Privilege Cloud uses the **same individual permission model** as on-prem CyberArk. Agent 03 maps all 22 safe member permissions directly via the Safe Members API — there is no role-based simplification or permission loss.
...
Phase P1 produces the analysis report. Phase P3 applies permissions to Privilege Cloud via `add_safe_member()` / `update_safe_member()`, handling 409 conflicts (member already exists) by falling back to update.
...
3. TRANSFORM      — Map fields to Privilege Cloud format, preserve all properties
...
|--------|-------------|--------|----------|
| **A. CyberArk Lab Instance** | Point `config.json` at a dedicated CyberArk PVWA dev/lab instance + Privilege Cloud test tenant. This is the standard approach for most PAM migrations. | Low (config only) | High — real APIs, real behavior |
| **B. Mock API Server** | Build a lightweight Flask/FastAPI service that emulates CyberArk PVWA and Privilege Cloud REST APIs with synthetic data. Runs locally. | Medium (~2-3 days) | Medium — tests pipeline logic, misses edge cases |
| **C. Unit Test Fixtures** | pytest suite with `unittest.mock` patching `CyberArkClient` and `CloudClient` methods using recorded API responses. | Medium (~2-3 days) | Medium — good for CI/CD, no integration testing |
...
```bash
python3 cli.py run P4 --config config.dev.json    # Dev PVWA + test Privilege Cloud tenant
python3 cli.py run P5 --config config.prod.json   # Prod PVWA + prod Privilege Cloud tenant
```
...
- Non-production Privilege Cloud tenant (request from CyberArk)
...
- pytest with `unittest.mock.patch` on `CyberArkClient` and `CloudClient`
...
| P2 | Infrastructure Preparation | Privilege Cloud tenant, network, service accounts |
...
- HTTPS is enforced for all Privilege Cloud connections
