...
15-agent AI orchestrator for CyberArk PAS on-prem → Privilege Cloud migration. Agents are sequenced by phase via `coordinator.py`. Each agent extends `AgentBase`, returns `AgentResult`, and gates on prior agent output before proceeding. All actions are SHA-256 audit-logged.
...
| 05 | agent_05_heartbeat.py | Validator | 10 post-migration validation checks on target Privilege Cloud | False positive — account responds but CPM cannot rotate due to firewall block | Run `--deep-validate`; verify CPM network connectivity independently |
| 06 | agent_06_integration.py | Rewirer | CCP/AAM code scanning + repointing to Privilege Cloud endpoints | Misses hardcoded PVWA URLs in compiled binaries or obfuscated scripts | Manual review of flagged files; extend pattern list in scanner config |
...
| 13 | agent_13_platform_plugins.py | Porter | Export/import custom CyberArk platform plugins to Privilege Cloud | Custom platform with non-standard CPM plugin fails import silently — missing in target | Check `output/reports/platform_import.json` for `status: failed`; manual plugin rebuild |
...
| 15 | agent_15_hybrid_fleet.py | Balancer | Hybrid fleet manager during parallel running (on-prem + Privilege Cloud) | Split-brain: both CPM instances rotate same account simultaneously | Set `HYBRID_MODE=exclusive` in `agent_config.json`; enforce mutex lock |
...
Maps all 22 CyberArk safe permissions 1:1 to Privilege Cloud (no loss — same permission model). Flags 7 security-sensitive permissions for human review before P3.
...
Scans codebases for CCP/AAM patterns. Generates Privilege Cloud replacement code. Supports Python, Java, .NET, PowerShell, Bash.
...
10 assertions against staging Privilege Cloud. Hard-blocks production on any failure. Supports rollback to on-prem baseline.
...
Export custom platforms from on-prem → validate → import to Privilege Cloud. Flags non-standard CPM plugins requiring manual rebuild.
...
Manages accounts split across on-prem and Privilege Cloud during parallel running. Routes CPM rotation to correct system. Tracks cutover readiness per account group.
