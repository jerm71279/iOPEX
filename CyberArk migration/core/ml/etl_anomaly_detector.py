...
class ETLAnomalyDetector:
    ...
    def retrain(self) -> Dict[str, Any]:
        ...
        Call this after each wave completes.  Returns training metadata.
        """
    ...
    def load_state(self, state: Dict):
        ...
        self._ewma_mean = state.get("ewma_mean", {})
        self._ewma_var = state.get("ewma_var", {})
        self._sample_count = state.get("sample_count", {s: 0 for s in ETL_STEPS})
    ...
    def _predict_if(self, record: StepRecord) -> float:
        ...
        sklearn's decision_function returns negative for outliers,
        so we normalise: lower → more anomalous → higher score.
        """
        import numpy as np
        ...
        # decision_function: large negative = outlier, positive = inlier
